...
def main():
    while(True):
         ...
         device_data = requests.get("http://" + host + ":6002/scheduler/devices/tasks/transmitter?max_seconds=5", headers={'relia-device': device_id, 'relia-password': password}).json()
         if device_data.get('taskIdentifier'):
              ...
              device_data = requests.post("http://" + host + ":6002/scheduler/devices/tasks/transmitter/" + device_data.get('taskIdentifier'), headers={'relia-device': device_id, 'relia-password': password}).json()
